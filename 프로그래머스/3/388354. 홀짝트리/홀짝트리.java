...
class Solution {
    ...
    public int[] solution(int[] nodes, int[][] edges) {
        ...
        // for(Integer c : count.keySet()) System.out.println(c + " " + count.get(c) + " ");
        // System.out.println();
        ...
        for(int i = 0; i < nodes.length; i++) {
            ...
            if(isValid(root, root, visited)) {
                ...
                // System.out.println("루트 노드 검증 성공 + " + root);
    ...
    static boolean solve(int root, Set<Integer> visited) {
        ...
        while(!stk.isEmpty()) {
            ...
            // leaf node인 경우, 무조건 노드 번호는 짝수여야 한다. 자식노드가 없기 때문에
            // if(nodeList[map.get(p)].childs.size() == 1 && visited.contains(nodeList[map.get(p)].childs.get(0))) {
